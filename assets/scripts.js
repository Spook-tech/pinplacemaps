...
if (document.getElementById('color-palette')) {
    ...
    // Делегирование событий через body
    document.body.addEventListener('click', function(event) {
          const buttonPalette = document.getElementById('color-palette');
    const colorPalette = document.querySelector('.color-options');
    const selectedColorName = document.getElementById('selected-color-name');
    ...
    const currentColorElement = document.querySelector('#current-color-hidden');
        ...
        // Обработка клика по кнопке палитры
        if (event.target.id === 'color-palette') {
            event.target.classList.toggle('active');
            colorPalette.classList.toggle('show');
        }
        
        // Обработка клика по опции цвета
        if (event.target.classList.contains('color-option')) {
            // Удаляем класс active со всех опций цвета
            const allColorOptions = document.querySelectorAll('.color-option');
            allColorOptions.forEach(opt => opt.classList.remove('active'));
            
            // Добавляем active к выбранной опции
            event.target.classList.add('active');
            
            // Обновляем текст и круг выбранного цвета
            const swatchElement = event.target.querySelector('.swatch');
            if (swatchElement) {
                const color = swatchElement.style.cssText;
                console.log(color);
                ...
                if (match) {
                    ...
                    currentColorElement.dataset.color = rgbColor;
                    selectedColorCircle.style.backgroundColor = rgbColor;
                }
            }
        }
    });
