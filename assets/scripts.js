...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (form) {
    form.addEventListener('submit', function (event) {
      ...
      .then(data => {
        // Очистите предыдущие сообщения об ошибках и успехах
        ...
        if (data.status === 'success') {
          ...
          successMessage.innerHTML = `
            <span class="svg-wrapper">
              ${data.icon_success} <!-- Вставка иконки успеха -->
            </span>
            ${data.message}
          `;
          ...
          errorMessage.className = 'newsletter-form__message form__message';
          errorMessage.innerHTML = `
            <span class="svg-wrapper">
              ${data.icon_error} <!-- Вставка иконки ошибки -->
            </span>
            ${data.errors.join(', ')}
          `;
